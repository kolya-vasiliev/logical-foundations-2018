...
Proof. induction n as [|n].
  ...
  rewrite evenb_S. destruct IHn as [k K]. destruct (evenb n) eqn:H. 
    - exists k. simpl. rewrite <- K. reflexivity.  
    - exists (S k). simpl. rewrite <- K. reflexivity.
Qed.
...
Proof. intros x y. rewrite <- beq_nat_true_iff. split.
  - intros H H2. rewrite H in H2. inversion H2.
...
Proof. intros A beq H l1. induction l1 as [|h l1 IH].
  - intros l2.  split. 
    + destruct l2. reflexivity. intros H2. inversion H2.
    + intros H2. rewrite <- H2. reflexivity.
  - intros l2. split.
    + destruct l2. simpl. intros H2. inversion H2.
      simpl. rewrite andb_true_iff, H. intros [H2 H3].
      apply IH in H3. rewrite H2, H3. reflexivity. 
    + intros H2.  destruct l2. inversion H2.
      simpl. rewrite andb_true_iff. split.
        rewrite H. inversion H2. reflexivity.
        apply IH. inversion H2. reflexivity. Qed.
...
Proof. intros p. intros P. intros H.
  assert (H2: (P->False) -> P).
    {intros H'. exfalso. apply H. unfold not. apply H'. } 
    apply p with (Q:=False). apply H2. Qed.
...
Theorem dne_implies_em: double_negation_elimination -> excluded_middle.
Proof. intros dne. intros P. apply (dne (P\/~P)). 
  apply excluded_middle_irrefutable. Qed.
...
Proof. intros em. intros P Q. destruct (em P) as [p|np].
  intros pq. right. apply pq. apply p. 
  intros pq. left. apply np. Qed.
