...
Lemma t_update_eq : forall A (m: total_map A) x v,
  (m & {x --> v}) x = v.
Proof. intros. unfold t_update. rewrite <- beq_string_refl. reflexivity. Qed.
...
Theorem t_update_neq : forall (X:Type) v x1 x2
  ...
  x1 <> x2 ->
  (m & {x1 --> v}) x2 = m x2.
Proof. intros. apply false_beq_string in H. unfold t_update. rewrite H.
  reflexivity. Qed.
...
Lemma t_update_shadow : forall A (m: total_map A) v1 v2 x,
    m & {x --> v1 ; x --> v2} = m & {x --> v2}.
Proof. intros. apply functional_extensionality. intros a.
  unfold t_update. destruct (beq_string x a). 
  - reflexivity.
  - reflexivity.
Qed.
...
Proof. unfold beq_string. intros x y. destruct (string_dec x y) as [H|H].
  constructor. apply H.
  constructor. apply H.
Qed.
...
Theorem t_update_same : forall X x (m : total_map X),
  ...
  intros a. unfold t_update.
  destruct (beq_stringP x a).
    subst. reflexivity. 
    reflexivity. Qed.
...
Theorem t_update_permute : forall (X:Type) v1 v2 x1 x2
  ...
  m & { x2 --> v2 ; x1 --> v1 }
  =  m & { x1 --> v1 ; x2 --> v2 }.
Proof. intros. apply functional_extensionality. unfold t_update.
  intros a. destruct (beq_stringP x1 a).
  - subst. apply false_beq_string in H. rewrite H.
    reflexivity.
  - reflexivity. Qed.
