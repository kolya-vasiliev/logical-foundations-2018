...
Example orand: (andb true true) = (orb true false).
Proof. simpl. reflexivity. Qed.
...
Proof.
  intros n m H.
  rewrite -> plus_1_l, <- H. reflexivity. Qed.
...
(**
Fixpoint evil_zero (n m : nat) : nat :=
  match n, m with
  | O, O => O
  | O, S m' => evil_zero O m'
  | S n', m' => evil_zero n' (S m')
  end.
*)
...
Theorem negb_twice_idemp : forall (f:bool->bool),
  (forall x:bool, f x = negb x) -> forall (b : bool), f (f b) = b.
Proof. 
  intros f H b. rewrite H, H. destruct b. 
  - reflexivity.
  - reflexivity. Qed.
...
Example test_bin_incr1: bin_to_nat (dbl (dbl (dbl1 zero))) = 4.
Proof. reflexivity. Qed.

Example test_bin_incr2: bin_to_nat (dbl (dbl (dbl (dbl1 (dbl (dbl zero)))))) = 8.
Proof. reflexivity. Qed.

Example test_bin_incr3: bin_to_nat (dbl1 zero) = 1.
Proof. reflexivity. Qed.

Example test_bin_incr4: bin_to_nat (dbl (dbl (dbl (dbl zero)))) = 0.
Proof. reflexivity. Qed.

Example test_bin_incr5: bin_to_nat (dbl1 (dbl1 (dbl1 (dbl1 zero)))) = 15.
Proof. reflexivity. Qed.
