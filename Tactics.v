...
The [rewrite] tactic doesn't try to finish the goal, it just performs 
some substitutions on it, typically you use as a part of a chain 
of substitutions and simplifications when you far away from final result. 
[apply] is better used when the goal matches the conclusion exactly.  
...
Proof.
  ...
  - destruct m.
    + intros H. inversion H.
    + intros H. 
      rewrite <- plus_n_Sm, <- plus_n_Sm in H. inversion H.
      apply IHn' in H1. inversion H1. reflexivity. Qed.
...
Proof. induction n as [|n IH].
  ...
  - intros m H. destruct m.
    + inversion H.
    + apply IH in H. inversion H. reflexivity. Qed.
...
Theorem nth_error_after_last: forall (n : nat) (X : Type) (l : list X),
     length l = n ->
     nth_error l n = None.
Proof. intros n X l. generalize dependent n. induction l as [|h l IH].
  - reflexivity.
  - intros [|n].
    + intros H. inversion H.
    + simpl. intros H. apply IH. inversion H. reflexivity. Qed.
...
Proof. intros f b. 
  ...
  - destruct (f false) eqn:Hff.
    + destruct b eqn:Hb.
      rewrite Hft, Hft, Hft. reflexivity. 
      rewrite Hff, Hft, Hft. reflexivity.  
    
    + destruct b eqn:Hb.
      rewrite Hft, Hft, Hft. reflexivity.
      rewrite Hff, Hff, Hff. reflexivity.
  ...
  - destruct (f false) eqn:Hff.
    ...
    + destruct b eqn:Hb.
      rewrite Hft, Hff, Hff. reflexivity.
      rewrite Hff, Hff, Hff. reflexivity.
Qed.
...
Proof. intros n m. destruct (beq_nat n m) eqn: Hnm.
  - destruct (beq_nat m n) eqn: Hmn.
    + reflexivity.
    + apply beq_nat_true in Hnm. rewrite Hnm in Hmn.
      rewrite <- beq_nat_refl in Hmn. inversion Hmn.
  - destruct (beq_nat m n) eqn: Hmn.
    + apply beq_nat_true in Hmn. rewrite Hmn in Hnm.
      rewrite <- beq_nat_refl in Hnm. inversion Hnm.
    + reflexivity. Qed.
...
Theorem beq_nat_trans : forall n m p,
  ...
  beq_nat m p = true ->
  beq_nat n p = true.
Proof. intros n m p Hnm Hmp.
  apply beq_nat_true in Hnm. apply beq_nat_true in Hmp.
  assert (H: n=p).
    {apply eq_trans with m. apply Hnm. apply Hmp. }
...
Theorem length_empty : forall X (l:list X),
  O = length l -> l = [].
...
Proof. intros X l1. induction l1 as [|h1 l1 IH].
  - simpl. intros l2 H. apply length_empty in H. rewrite H. reflexivity. 
  - intros l2 H. destruct l2 as [|h2 l2]. 
    + inversion H.
    + simpl in H. inversion H. apply IH in H1. simpl. rewrite H1.
      reflexivity. Qed.
...
Theorem existsb_existsb': forall X P (l:list X),
  existsb P l = existsb' P l.
Proof. intros X P l. induction l as [|h l IH].
  - reflexivity.
  - simpl. unfold existsb'. simpl. destruct (P h).
    + reflexivity.
    + simpl. apply IH. Qed.
