...
Check (forall (P:rgb -> Prop), P red -> P green -> P blue -> forall p:rgb, P p).
...
Check (forall (P:byntree->Prop), 
     P bempty -> 
     (forall (b:yesno), P (bleaf b)) -> 
     (forall (b:yesno) (x:byntree), (P x) -> forall (y:byntree), (P y) -> 
     P(nbranch b x y)) ->
...
Check (forall (X:Type) (P : tree X -> Prop),
...
Definition P_pc (n:nat) := forall m, n + m = m + n.

Print P_pc.

Theorem plus_comm''' : forall n, (P_pc n).
Proof. apply nat_ind.
  - intros m. rewrite <- plus_n_O. reflexivity.
  - unfold P_pc. intros n H m. rewrite <- plus_n_Sm.
    rewrite <- (H m). reflexivity.
Qed.

Definition P_pa (n:nat) := forall m p : nat,
  n + (m + p) = (n + m) + p.
  
Theorem plus_assoc'' : forall n, P_pa n.
Proof. apply nat_ind.
  - intros m p. reflexivity.
  - unfold P_pa. intros n H m p. simpl. rewrite H. reflexivity. Qed.
