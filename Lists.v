...
Fixpoint nonzeros (l:natlist) : natlist :=
  ...
  | h :: t => 
    if beq_nat O h 
      then (nonzeros t) else (h :: (nonzeros t))
  end.
...
Theorem bag_theorem : forall v l, 
  S (count v l) = count v (add v l). 
Proof. intros v l. destruct l.
...
Proof. intros l1 l2. induction l1 as [|v l1 IH]. 
  - simpl. rewrite app_nil_r. reflexivity.
  - simpl. rewrite IH, app_assoc. reflexivity. Qed.
...
Proof. induction l as [|v l IH].
  - reflexivity.
  - simpl. rewrite <- beq_nat_refl, IH. reflexivity. Qed.
...
Theorem count_additive: forall s1 s2 v, 
  count v (sum s1 s2) = (count v s1) + (count v s2). 
Proof. intros s1 s2 v. induction s1 as [|h s1 IH].
  - reflexivity. 
  - simpl. destruct (beq_nat v h).
...
Proof. intros l d.  destruct l as [|h l IH]. 
  - reflexivity.
  - reflexivity. Qed.
...
Proof. destruct x. rewrite (beq_nat_refl n). reflexivity. Qed.
...
Proof. intros d x v. simpl. rewrite <- beq_id_refl.  reflexivity. Qed.
(** [] *)
...
Proof. intros d x y o H. simpl. rewrite H. reflexivity. Qed.
